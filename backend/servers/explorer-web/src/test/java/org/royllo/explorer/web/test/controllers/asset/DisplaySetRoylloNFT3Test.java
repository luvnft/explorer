...
public class DisplaySetRoylloNFT3Test extends BaseWebTest {

    // Asset tested.
    String assetId = SET_OF_ROYLLO_NFT_3_ASSET_ID;
    String assetIdAlias = SET_OF_ROYLLO_NFT_3_ASSET_ID_ALIAS;
    DecodedProofValueResponse.DecodedProof assetFromTest = SET_OF_ROYLLO_NFT_3_FROM_TEST.getDecodedProofResponse(0);
    ...
    void assetPageOwner(final HttpHeaders headers) throws Exception {
        ...
        mockMvc.perform(get("/asset/" + assetId + "/owner").headers(headers))
                ...
                .andExpect(content().string(containsString(">" + NEW_USER_ID + "<")))
                .andExpect(content().string(containsString(">" + NEW_USER_USERNAME + "<")))
    ...
    void assetPageProofs(final HttpHeaders headers) throws Exception {
        ...
        mockMvc.perform(get("/asset/" + assetId + "/proofs").headers(headers))
                ...
                .andExpect(content().string(not(containsString(">" + SET_OF_ROYLLO_NFT_1_FROM_TEST.getDecodedProofRequest(0).getProofId() + ".proof<"))))
                .andExpect(content().string(not(containsString(">" + SET_OF_ROYLLO_NFT_2_FROM_TEST.getDecodedProofRequest(0).getProofId() + ".proof<"))))
                .andExpect(content().string(containsString(">" + SET_OF_ROYLLO_NFT_3_FROM_TEST.getDecodedProofRequest(0).getProofId() + ".proof<")))
                // Error messages.
                .andExpect(content().string(not(containsString(getMessage(messages, "asset.view.error.noAssetId")))))
                .andExpect(content().string(not(containsString(getMessage(messages, "asset.view.error.assetNotFound")))));
