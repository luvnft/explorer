...
@SpringBootTest
@DisplayName("Display unlimitedRoylloCoin1 page tests")
@AutoConfigureMockMvc
public class DisplayUnlimitedRoylloCoin1Test extends BaseWebTest {

    // Asset tested.
    String assetId = UNLIMITED_ROYLLO_COIN_1_ASSET_ID;
    String assetIdAlias = UNLIMITED_ROYLLO_COIN_1_ASSET_ID_ALIAS;
    DecodedProofValueResponse.DecodedProof assetFromTest = UNLIMITED_ROYLLO_COIN_1_FROM_TEST.getDecodedProofResponse(0);
    ...
    void assetPageProofs(final HttpHeaders headers) throws Exception {
        ...
        mockMvc.perform(get("/asset/" + assetId + "/proofs").headers(headers))
                ...
                .andExpect(content().string(containsString(">" + UNLIMITED_ROYLLO_COIN_1_FROM_TEST.getDecodedProofRequest(0).getProofId() + ".proof<")))
                // Error messages.
                .andExpect(content().string(not(containsString(getMessage(messages, "asset.view.error.noAssetId")))))
                .andExpect(content().string(not(containsString(getMessage(messages, "asset.view.error.assetNotFound")))));
